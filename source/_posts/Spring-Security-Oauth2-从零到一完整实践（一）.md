---
layout: post
title: Spring Security Oauth2 从零到一完整实践（一）
date: 2021-04-19 09:07:50
tags:
---
...
![](/images/Spring-Security-Oauth2-从零到一完整实践（一）/1.png)
...
![](/images/Spring-Security-Oauth2-从零到一完整实践（一）/2.png)
...
![](/images/Spring-Security-Oauth2-从零到一完整实践（一）/3.png)

用户通过浏览器请求前端应用的页面，然后页面里面加载请求到数据，再渲染页面。那么现在的授权没有 session 了，前后端是完全独立的两个项目了，我们要怎么进行认证授权呢？对于一个受保护的应用来说，他的请求流程如下：

![](/images/Spring-Security-Oauth2-从零到一完整实践（一）/4.png)

在这个流程中，我们后端应用其实变成了两个，一个是`授权服务器`，一个是`资源服务器`，**当然你完全可以简单把他们两个同时写在一个之中。**单独提出来的好处是什么呢？最主要的一点就是上面提到的 **使第三方应用程序能够获得对 Web 服务的有限访问权限，简单的说就是能够更加方便的另外一个应用接入。当你写好一个授权服务器以后，其他应用就可以共用这个授权服务器，**他们就作为资源服务器亦或是客户端即可。
...
|                   |          | invalid_client         | 客户端身份验证失败(例如，未知客户端、不包含客户端身份验证或不支持的身份验证方法)。授权服务器可以返回一个超文本传输协议401(未授权)状态码，以指示支持哪些超文本传输协议认证方案。如果客户端试图通过“授权”请求头字段进行身份验证，授权服务器必须用一个HTTP 401(未授权)状态代码进行响应，并包括与客户端使用的身份验证方案相匹配的“WWW-Authenticate”响应头字段。
...
![](/images/Spring-Security-Oauth2-从零到一完整实践（一）/5.png)
![](/images/Spring-Security-Oauth2-从零到一完整实践（一）/6.png)
![](/images/Spring-Security-Oauth2-从零到一完整实践（一）/7.png)
![](/images/Spring-Security-Oauth2-从零到一完整实践（一）/8.png)
![](/images/Spring-Security-Oauth2-从零到一完整实践（一）/9.png)
